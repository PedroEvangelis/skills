...
| Need | Diagram Type | Mermaid Type | Used In |
|------|--------------|--------------|---------|
| Data structures, domain model | Class Diagram | `classDiagram` | CP1 (Domain Discovery) |
| Entity relationships, database design | ER Diagram | `erDiagram` | CP3 (Conceptual Design) |
| API flow, protocols, interactions | Sequence Diagram | `sequenceDiagram` | CP3 (Conceptual Design) |
| Business processes, workflows | Activity Diagram | `flowchart` | CP3 (Conceptual Design) |
| Actor interactions, requirements | Use Case Diagram | *(not supported — use text table)* | CP2 (Spec) |
| Lifecycle, state transitions | State Machine | `stateDiagram-v2` | CP3 (Conceptual Design) |
| System structure, components | Component Diagram | `flowchart` with subgraphs | CP3 (Conceptual Design) |
| As-Is / To-Be process flows | Process Flow | `flowchart` with subgraphs | CP3 (Conceptual Design) |
...
| Inconsistent naming across diagrams | Use the ubiquitous language glossary |
